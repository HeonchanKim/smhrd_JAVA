...
public class PersonMain {
	public static void main(String[] args) {
		...
		//업캐스팅이 되었을 때는 그 전 상태(하위클래스)의 객체 멤버에는
		//접근할수 없음 단, 오버라이딩된 경우에는 하위클래스의 메서드 호출
		Person p = s1;
		p.walk(); //동적바인딩
		// System.out.println(p.stdId); // 오류!
		
		System.out.println("=======================");
		Student p1 = new Student("김헌찬");
		Teacher p2 = new Teacher("전채원");
		//서로 타입이 다르기 때문에 배열이나 리스트에 담을 수 없음
		...
		//업캐스팅 된 상태에서 원래 어떤 타입이었는지 알고 싶을 떄 사용하는 연산자
